...
function getUsage(command?: SondeCommand): string {
  if (!command) {
    return [
      ...
      "",
      "Sonde CLI for manifest generation, evaluation reporting, and self-compliance.",
      "",
      "Commands:",
      "  generate <cli>    Generate a Sonde manifest by exploring <cli> help output",
      "  score <cli>       Evaluate <cli> against manifest rules and return report",
      "  manifest          Return Sonde's own Sondage manifest (self-compliance)",
      "  run <cli>         Run deterministic diagnostic checks from local manifest",
      "  publish <cli>     Optionally submit evaluation report to Sonde web API",
      "  serve             Optional JSON-line protocol for tool integrations",
      "",
      "Options:",
